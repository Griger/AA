...
Como podemos ver con `summary()`, con las 14 primeras componentes principales estamos explicando un 96% de los datos, y son con las que nos vamos a quedar para hacer el estudio reducido y ver si hay mejora al utilizar PCA. Vamos a hacer entonces la combinación lineal que nos da PCA para obtener el nuevo conjunto de train. Vamos a ver el PCA para 10 clases:

```{r}
summary(pca.out10)
```

De nuevo con las 14 primeras componentes principales conseguimos explicar más del 96% de los datos, con lo que vamos a quedarnos también con las 14 primeras.
...
```{r}
prediccionesPCA <- predictOVA(trainPCA10, CLASS.train, 10, testPCA10)
cat("Porcentaje de acierto con 10 clases y PCA:", 
    100*sum(prediccionesPCA == CLASS.test)/length(CLASS.test))
```

```{r}
predicciones <- predictOVA(train10, CLASS.train, 10, test10)
cat("Porcentaje de acierto con 10 clases sin PCA:", 
    100*sum(predicciones == CLASS.test)/length(CLASS.test))
```

En este caso los porcentajes de acierto son más bajos y sí hay casi un 10% de diferencia entre utilizar PCA y no utilizarlo, con lo que descartamos para 10 clases usar PCA en el caso de paramétricos.
...
predicciones <- predictEt(train, NSP.train, test, 3, best_r)
acierto <- sum(predicciones == NSP.test)/length(predicciones[predicciones != 0])
no_clasificados <- length(predicciones[predicciones == 0])/length(predicciones)
...
predicciones <- predictEt(trainPCA, NSP.train, testPCA, 3, best_r)
acierto <- sum(predicciones == NSP.test)/length(predicciones[predicciones != 0])
no_clasificados <- length(predicciones[predicciones == 0])/length(predicciones)
...
predicciones <- predictEt(trainPCA10, CLASS.train, testPCA10, 10, best_r)
acierto <- sum(predicciones == CLASS.test)/
  length(predicciones[predicciones != 0])
no_clasificados <- length(predicciones[predicciones == 0])/length(predicciones)
...
predicciones <- predictEt(train10, CLASS.train, test10, 10, best_r)
acierto <- sum(predicciones == CLASS.test)/
  length(predicciones[predicciones != 0])
no_clasificados <- length(predicciones[predicciones == 0])/length(predicciones)

...
mpg1.train = sapply(datos.train$mpg, function(x) if (x < mediana) return(-1) else return(1))
mpg1.test = sapply(datos.test$mpg, function(x) if (x < mediana) return(-1) else return(1))
```
...
RL = glm(mpg01 ~ displacement+horsepower+weight, data = train)

prediccion = predict(RL, newdata = test, se.fit = TRUE)
```
...
KNN = knn(train.norm, test.norm, train$mpg01, k = 3)

tune.knn(rbind(train.norm, test.norm), as.factor(c(mpg1.train,mpg1.test)), k=1:100, tune.control(sampling = "cross"), cross = 10)
```
